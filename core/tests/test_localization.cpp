...
auto main() -> int {
  ...
  "TranslationManager singleton"_test = [] -> void {
    const TranslationManager& transMgr = GetTranslationManager();
  ...
  "Get key fallback"_test = [] -> void {
    ...
    const String val = transMgr.get("NON_EXISTENT_KEY_XYZ_123");
    expect(val == "NON_EXISTENT_KEY_XYZ_123");
  };

  "Localization helpers"_test = [] -> void {
    const String val = _("NON_EXISTENT_KEY_HELPER");
    expect(val == "NON_EXISTENT_KEY_HELPER");
  };

  "Formatting helpers"_test = [] -> void {
    i32    arg = 123;
    const String val = _format("KEY_{}", arg);
    expect(val == "KEY_123");
  };
